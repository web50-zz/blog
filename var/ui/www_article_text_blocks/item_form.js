ui.www_article_text_blocks.item_form = Ext.extend(Ext.form.FormPanel, {
	Load: function(data){
		var f = this.getForm();
		f.load({
			url: 'di/www_article_text_blocks/get.json',
			params: {_sid: data.id},
	...
	Save: function(){
		...
		if (f.isValid()){
			f.submit({
				url: 'di/www_article_text_blocks/set.do',
	...
	constructor: function(config){
						...
						store: new Ext.data.JsonStore({url: 'di/www_article_text_blocks_types/type_list.json', root: 'records', fields: ['id', 'title'], autoLoad: true,
							listeners: {
								load: function(store,ops){
									var f = this.getForm().findField('block_type');
		...
		ui.www_article_text_blocks.item_form.superclass.constructor.call(this, config);
		this.on({
