ui.www_article_text_blocks.main = Ext.extend(ui.www_article_text_blocks.grid, {
	Load: function(data){
		this.setParams({}, true);
	},
	Add: function(){
		...
		app.on({
			apploaded: function(){
				var f = new ui.www_article_text_blocks.item_form();
		...
		app.Load('www_article_text_blocks', 'item_form');
	},
	Edit: function(){
		var app = new App({waitMsg: 'Загрузка формы'});
		app.on({
	...
	constructor: function(config)
	{
		...
		Ext.apply(this, {
			...
			tbar: [
				...
				'->', {iconCls: 'help', handler: function(){showHelp('www_article_text_blocks')}}
			]
		});
		Ext.apply(this, {
			listeners: {
				...
				rowmove: function(target, row){
					Ext.Ajax.request({
						url: 'di/www_article_text_blocks/reorder.do',
						method: 'post',
		...
		ui.www_article_text_blocks.main.superclass.constructor.call(this, config);
	},
